...
start_link(Center_url, Responder_mod, Pull_delay) ->
  crypto:start(),
  ssh:start(temporary),
  supervisor:start_link({local, ?MODULE}, ?MODULE, [Center_url, Responder_mod, Pull_delay]).
...
init([Center_url, Responder_mod, Pull_delay]) ->
